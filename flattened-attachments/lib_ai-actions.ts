// lib/ai-actions.ts (updated to remove unsupported 'file_search' tool for Grok compatibility)
  ...
  for (const url of fileUrls) {
    try {
      ...
      try {
        ...
        extractedText = ''; // Fallback to empty
      }
...
Field: "risks": array of strings (single warning or empty)`;
      ...
      if (focus === 'contract') {  // Updated to match Phase 1A focus
      ...
      // Replace {fields} in baseInstructions with the built jsonFields
      prompt = prompt.replace('{fields}', `{${jsonFields}}`);

      // Add extracted text to prompt
      prompt += `\nExtracted text from PDF: ${extractedText.substring(0, 2000)}`; // Truncate to avoid token limits

      // Grok completion call (removed unsupported 'file_search' tool; rely on extracted text in prompt)
      ...
      // Handle "Null" strings as null for consistency
