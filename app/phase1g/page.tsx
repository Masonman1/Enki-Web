  ...
  const handleUpload = async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      const uploadPromises = acceptedFiles.map(async (file) => {
        ...
        const path = `cos/user_${userId}/${safeName}`;
        ...
        const { data, error: uploadError } = await supabase.storage
          ...
          .upload(path, file, {
            ...
            metadata: { type: 'cos', jobId: '7b078984-527a-495f-a738-18a0fa53de35', deleted_at: null }
          });
      ...
      // Stub: Insert to cos JSONB in jobs
      const cos = parsedRisks.map((r, idx) => ({ id: `co-${idx}`, description: r, clause: clauses[idx] }));
      console.log('Stub: Adding to jobs.cos:', cos);
      // Real: await supabase.from('jobs').update({ cos: [...existing, ...cos] }).eq('id', '7b078984-527a-495f-a738-18a0fa53de35');

    } catch (err) {
      setError(err.message || 'Process failed.');
      toast.error('Error during change order handling.');
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center p-24">
      <Card className="w-[600px]">
        <CardHeader>
          <CardTitle>Phase 1G: Change Orders</CardTitle>
          <CardDescription>Upload change order docs for risk parsing and protective clause generation.</CardDescription>
        </CardHeader>
        <CardContent>
